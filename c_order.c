...
#define RECORD_MAX_SIZE 90010 
#define RECORD_MAX_LENGTH 100 
...
static int get_order(struct order **p_order_arr){ //?????????????????????????????????
	char file_name[] = "c_order.csv";  //?????????????????????????????????
    FILE *fp;  
    ...
    *p_order_arr=(struct order *)malloc(index*sizeof(struct order));//?????????????????????????????????
    ...
    for(i=1;i<index;i++){
      ...
      (*p_order_arr)[i].O_ID=atol(token);//?????????????????????????????????
      
      token=strtok(NULL, ",");
      (*p_order_arr)[i].O_D_ID=atol(token);//?????????????????????????????????
      
      token=strtok(NULL, ",");
      (*p_order_arr)[i].O_W_ID=atol(token);//?????????????????????????????????
	  
	    token=strtok(NULL, ",");
      (*p_order_arr)[i].O_C_ID=atol(token);//?????????????????????????????????
	  
	    token=strtok(NULL, ",");
      (*p_order_arr)[i].O_ENTRY_DATE=atol(token);//?????????????????????????????????
	  
	    token=strtok(NULL, ",");
      (*p_order_arr)[i].O_CARRIER_ID=atol(token);//?????????????????????????????????
	  
	    token=strtok(NULL, ",");
      (*p_order_arr)[i].O_OL_CNT=atoi(token);//???????????????????????????

      token=strtok(NULL, ",");
      (*p_order_arr)[i].O_ALL_LOCAL=atoi(token);//???????????????????????????
